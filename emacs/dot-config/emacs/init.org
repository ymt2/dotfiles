...
(defconst my/org-roam-directory
  (file-truename "~/Library/CloudStorage/SynologyDrive-home/org/"))

(setq org-roam-db-location (expand-file-name "~/.cache/org-roam/personal.db"))

(defconst my/emacs-server-directory
  (expand-file-name "emacs/server"
                    (or (getenv "XDG_CACHE_HOME") "~/.cache"))
  "Directory for Emacs server socket.")
#+end_src
...
(use-package eshell
  ...
  (dnd-protocol-alist '(("^file:" . my/dnd-insert-file-path))))
#+end_src
...
(use-package org-agenda
  ...
  (org-agenda-files (list my/org-roam-directory))
  (org-todo-keywords
   '((sequence "BACKLOG(b)" "TODO(t)" "IN-PROGRESS(p)" "IN-REVIEW(r)" "|" "DONE(d)" "CANCELED(c)"))))
#+end_src
...
(use-package org-roam
  ...
  (org-roam-node-display-template
   (concat "${title:*} "
	   (propertize "${directories:20}" 'face 'org-tag)
...
(use-package personal
  :demand t
  :bind (("C-a" . my/beginning-of-line-or-indentation)))
#+end_src
...
(use-package server
  ...
  :no-require
  :config
  (setq server-socket-dir my/emacs-server-directory)
